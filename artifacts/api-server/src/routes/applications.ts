...
function mapApp(app: Record<string, unknown>, student?: Record<string, unknown> | null, opp?: Record<string, unknown> | null, handler?: Record<string, unknown> | null) {
  return {
    ...app,
    createdAt: toISO(app.createdAt),
    updatedAt: toISO(app.updatedAt),
    student: student ? { id: student.id, name: student.name, email: student.email, nationality: student.nationality, educationLevel: student.educationLevel, gpa: student.gpa, whatsappNumber: student.whatsappNumber } : null,
    opportunity: opp ? { id: opp.id, title: opp.title, slug: opp.slug, deadline: opp.deadline ? String(opp.deadline) : null, country: opp.country, category: opp.category } : null,
...
router.post("/", async (req, res) => {
  ...
  const { opportunityId, motivation, experience, contactPreference, whatsappNumber, contactTime, concerns } = req.body as Record<string, string>;
  if (!opportunityId) {
...
router.get("/admin/all", async (req, res) => {
  ...
  try {
    ...
    let query = db.select().from(managedApplicationsTable).orderBy(desc(managedApplicationsTable.createdAt));
    const apps = await (status
      ? db.select().from(managedApplicationsTable).where(eq(managedApplicationsTable.status, status as string)).orderBy(desc(managedApplicationsTable.createdAt))
      : query);
