...
import { validateListingForPublish } from "../lib/listingSeo.js";
...
router.put("/scraper/items/:id/approve", async (req, res) => {
  ...
  try {
    ...
    const normalizedDeadline = normalizeDeadline(deadline as string | null | undefined);
    const category = String(body.category ?? item.category ?? "Scholarships");
    const country = body.country ?? item.country;
    ...
    await db.transaction(async (tx: any) => {
      if (item.itemType === "scholarship") {
        ...
        const gate = validateListingForPublish({
          title,
          slug,
          description,
          content,
          ...
          hostOrganization: typeof body.hostOrganization === "string" ? body.hostOrganization : item.source,
          hostWebsite: typeof body.hostWebsite === "string" ? body.hostWebsite : null,
        });

        if (!gate.ok) {
          res.status(400).json({ error: "Listing failed publish quality gate", issues: gate.errors, warnings: gate.warnings });
          return;
        }

        // Persist cover image to our own storage so the URL is permanent
        // and served from our own domain (Phase 4 image pipeline).
        const rawCoverImage = coverImage || item.coverImage || null;
        let persistedCoverImage = rawCoverImage;
        if (rawCoverImage && rawCoverImage.startsWith("http")) {
          approvalStage = "persisting cover image";
          try {
            const persisted = await persistImage(rawCoverImage, slug);
            if (persisted) persistedCoverImage = persisted;
          } catch {
            // Non-fatal — fall back to the original source URL
          }
        }
        ...
        const rawJobCoverImage = coverImage || item.coverImage || null;
        let persistedJobCoverImage = rawJobCoverImage;
        if (rawJobCoverImage && rawJobCoverImage.startsWith("http")) {
          try {
            const persisted = await persistImage(rawJobCoverImage, `job-${Date.now()}`);
            if (persisted) persistedJobCoverImage = persisted;
          } catch {
            // Non-fatal — fall back to the original source URL
          }
        }
        const [job] = await tx
          ...
          .values({
            ...
            coverImage: persistedJobCoverImage,
...
router.get("/scraper/diagnose", async (req, res) => {
  ...
  try {
    const rows = await db.execute(
      sql.raw("SELECT count(*)::int AS total FROM scraped_items"),
    );
    report.rawRowCount = (rows as any).rows?.[0]?.total ?? (rows as any)[0]?.total ?? null;
  } catch (err) {
    report.rawRowCountError = err instanceof Error ? err.message : String(err);
  }

  try {
    const cols = await db.execute(
      sql.raw(
        "SELECT column_name FROM information_schema.columns WHERE table_name = 'scraped_items' ORDER BY column_name",
      ),
    );
    const list = ((cols as any).rows ?? cols) as { column_name: string }[];
    report.columns = list.map((c) => c.column_name);
  } catch (err) {
    report.columnsError = err instanceof Error ? err.message : String(err);
  }

  try {
    const items = await db.select().from(scrapedItemsTable).limit(1);
    report.selectAllOk = true;
    report.sampleReadable = items.length;
  } catch (err) {
    report.selectAllOk = false;
    report.selectAllError = err instanceof Error ? err.message : String(err);
  }
